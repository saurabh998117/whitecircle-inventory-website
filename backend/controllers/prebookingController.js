...
const submitPrebooking = async (req, res) => {
    const { businessName, userName, email, wantsDemo } = req.body;

    if (!businessName || !userName || !email) {
        return res.status(400).json({ message: 'Please fill all required fields: Business Name, Your Name, and Your Email.' });
    }
